...
//dCRQ_CATEGORY_ASSET_ARMAMENT = missionNamespace getVariable ["dCRQ_CATEGORY_ASSET_ARMAMENT", CRQ_CATEGORY_ASSET_ARMAMENT];
dCRQ_AMMO_DATA = missionNamespace getVariable ["dCRQ_AMMO_DATA", CRQ_AMMO_DATA];
//dCRQ_AMMO_TYPES = missionNamespace getVariable ["dCRQ_AMMO_TYPES", CRQ_AMMO_TYPES];
...
CRQ_CatalogTerminalInit = {
	{
		...
		if (CQM_ITEM_TERMINAL_EXCLUDE findIf {_name == _x} == -1 && {_categories isEqualTo [] || {CQM_ITEM_TERMINAL_INCLUDE findIf {_name == _x} != -1}}) then {
			switch ([CRQ_CDAT_SIDE, _data] call CRQ_CatalogArrayData) do {
				case CRQ_SIDE_BLUFOR: {_categories pushBack CRQ_CATALOG_TERMINAL_BLUFOR;};
				case CRQ_SIDE_IDFOR: {_categories pushBack CRQ_CATALOG_TERMINAL_OPFOR;};
				case CRQ_SIDE_OPFOR: {_categories pushBack CRQ_CATALOG_TERMINAL_IDFOR;};
				case CRQ_SIDE_CIVFOR: {_categories pushBack CRQ_CATALOG_TERMINAL_CIVFOR;};
				default {};
			};
		};
		if (_categories isNotEqualTo []) then {
...
CRQ_CatalogAssetAnalysis = {
	...
	private _side = switch (getNumber (_x >> "side")) do {
		case 0: {CRQ_SIDE_OPFOR};
		case 1: {CRQ_SIDE_BLUFOR};
		case 2: {CRQ_SIDE_IDFOR};
		case 3: {CRQ_SIDE_CIVFOR};
...
CRQ_CatalogTerminalAnalysis = {
	...
	if (_side != CRQ_SIDE_UNKNOWN) then {_data pushBack [CRQ_CDAT_SIDE, _side];};
	_data
};
...
CRQ_CatalogUtilAssetType = {
	...
	_categories pushBack (switch ([CRQ_CDAT_SIDE, _data] call CRQ_CatalogArrayData) do {
		case CRQ_SIDE_BLUFOR: {CRQ_CATALOG_ASSET_BLUFOR};
		case CRQ_SIDE_IDFOR: {CRQ_CATALOG_ASSET_IDFOR};
		case CRQ_SIDE_OPFOR: {CRQ_CATALOG_ASSET_OPFOR};
