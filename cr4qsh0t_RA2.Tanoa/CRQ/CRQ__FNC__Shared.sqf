...
CRQ_fnc_ClassActions = {
	if (_this isEqualType "") then {_this = pCQ_CFG_Vehicles >> _this;};
	("true" configClasses (_this >> "UserActions"))
};
	...
	(_list apply {
		...
		if (_type isNotEqualTo "") then {
			if (!(_type in gCQ_PROP_SIMPLE)) then {
				...
				{_canSimple = _canSimple && {_cfg call _x};} forEach [
					{count (_this call CRQ_fnc_ClassLights) < 1},
					{count (_this call CRQ_fnc_ClassLadders) < 1},
					{count (_this call CRQ_fnc_ClassActions) < 1}
				];
