...
CRA_fnc_LC_Generate = {
	...
	if (_bsClutter) then {
		...
		_lcNew setVariable [CRA_SVAR_LOCATION_CLUTTER, [_bsOffVec, _bsArea#0, if (_inAnchorObj isEqualTo objNull) then {[]} else {[_inAnchorObj]}, 24, gRA_PM_MN_Clutter] call CRQ_WorldClutter2D];
	};
...
CRA_fnc_LC_InitMain = {
	private _fnc_sync = {
		CRA_LOAD_NEXT(0, 0);
		pRA_Locations = gRA_LC_List apply {private _vec = _x getVariable [CRA_SVAR_VEC, [[0,0,0],0]]; [text _x, [_vec#0#0, _vec#0#1]]};
		publicVariable "pRA_Locations";
	};
